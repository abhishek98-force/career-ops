...
/**
 * Extract `\resumeSubItem{Category}{Items}` calls as skill slots. The value
 * remains the primary slot span for backward-compatible value-only patches;
 * label/labelSpan expose the category for adaptive skill organization.
 *
 * @param {string} tex
...
function extractSubItemValues(tex) {
  ...
  while (searchFrom < tex.length) {
    ...
    slots.push({
      kind: 'skill',
      label: tex.slice(openCat + 1, closeCat),
      labelSpan: { start: openCat + 1, end: closeCat },
...
/**
 * Extract the editable second argument of a two-argument macro. Skill macros
 * also expose their first argument as an editable label; project names remain
 * fixed.
 *
 * @param {string} tex
 * @param {string} macroName
...
function extractSecondArgValues(tex, macroName, kind) {
  ...
  while (searchFrom < tex.length) {
    ...
    if (kind === 'skill') {
      slot.label = tex.slice(openFirst + 1, closeFirst);
      slot.labelSpan = { start: openFirst + 1, end: closeFirst };
    }
    slots.push(slot);
...
/**
 * @param {string} tex
 * @param {Array<{id: string, text?: string, label?: string, remove?: boolean}>} patches
 * @param {Array<{id: string, span: {start: number, end: number}, labelSpan?: {start: number, end: number}}>} slots
...
export function applyPatches(tex, patches, slots, { escape = true } = {}) {
  ...
  const ordered = [...patches]
    .map(p => {
      ...
      if (!slot) return null;
      return { slot, text: p.text, label: p.label, remove: p.remove === true };
    })
    .filter(Boolean)
    .sort((a, b) => (b.slot.callSpan?.start ?? b.slot.span.start) - (a.slot.callSpan?.start ?? a.slot.span.start));

  let out = tex;
  for (const { slot, text, label, remove } of ordered) {
    if (remove) {
    ...
    if (label !== undefined && !slot.labelSpan) {
      throw new Error(`Slot ${slot.id} does not expose an editable label`);
    }
    const replacements = [];
    if (typeof text === 'string') replacements.push({ span: slot.span, value: text });
    if (typeof label === 'string') replacements.push({ span: slot.labelSpan, value: label });
    replacements.sort((a, b) => b.span.start - a.span.start);
    for (const replacement of replacements) {
      const value = escape ? escapeLatex(replacement.value) : replacement.value;
      out = out.slice(0, replacement.span.start) + value + out.slice(replacement.span.end);
    }
  }
  return out;
}
